...
<script>
    jQuery(document).ready(function ($) {
        var cityData = {
            city: [<?php while($cities->have_posts()): $cities->the_post();global $post;$maps = get_field('maps');?>{
                    name: "<?php the_title();?>",
                    slag: "<?= $post->post_name;?>",
                    ...
                    $m = 1;
                    if($m==1):?>
                    latCenter: <?= $map['map']['lat'];?>,
                    lngCenter: <?= $map['map']['lng'];?>,
        ...
        function initialize(latCenter, lngCenter, places) {
            ...
            var markerImage = new google.maps.MarkerImage("<?= get_template_directory_uri();?>/assets/img/geomark.png", new google.maps.Size(56, 56), new google.maps.Point(0, 0));
            var infos = [];
            var contentTxt;
